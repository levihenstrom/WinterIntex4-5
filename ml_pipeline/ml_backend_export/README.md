...
(resolved from **repository root** = parent of `ml_pipeline/`; override with env `INTEX_REPO_ROOT` if needed.)
...
> `PYTHONPATH=. python3 -m ml_backend_export.run_all_backend_exports` only works if the current directory is `ml_pipeline` (so `ml_backend_export` is importable).
...
```bash
cd ml_pipeline && PYTHONPATH=. python3 -m ml_pipeline_reintegration_readiness_scorer.score_all_current_residents
```
