...
import { Search, Layers, Home, LayoutGrid, TrendingUp, CheckCircle, Clock, RotateCcw } from 'lucide-react';
...
/* ── Config ──────────────────────────────────────────────────── */
const STATUS_CFG: Record<AllocStatus, { bg: string; text: string; Icon: React.ElementType }> = {
  Allocated: { bg: '#DBEAFE', text: '#1E40AF', Icon: Clock },
  Spent:     { bg: '#DCFCE7', text: '#166534', Icon: CheckCircle },
  Returned:  { bg: '#FEF9C3', text: '#854D0E', Icon: RotateCcw },
};
        ...
        return (
          <div key={h} style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', border: '1px solid #E2E8F0', boxShadow: '0 2px 8px rgba(30,58,95,0.06)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
              <span style={{ color: '#166534', display: 'flex', alignItems: 'center', gap: 3 }}><CheckCircle size={10} /> ${spent.toLocaleString()}</span>
              <span style={{ color: '#1E40AF', display: 'flex', alignItems: 'center', gap: 3 }}><Clock size={10} /> ${alloc.toLocaleString()}</span>
            </div>
          </div>
        );
      })}
            ...
            <div key={r.name}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                ...
                <span style={{ color: '#1E3A5F', fontWeight: 700 }}>
                  ${r.total.toLocaleString()} <span style={{ color: '#94A3B8', fontWeight: 400 }}>({pct}%)</span>
                </span>
              </div>
      ...
      <div className="container">
        ...
        <div style={{
          ...
          <div style={{ marginLeft: 'auto', fontSize: 12, color: '#64748B', display: 'flex', alignItems: 'center', gap: 6 }}>
            <TrendingUp size={12} color="#1E3A5F" />
          ...
          {(['All', ...ALL_STATUSES] as string[]).map(s => {
            ...
            return (
              <button key={s} onClick={() => setStatusFilter(s)} style={{
                ...
                {SIcon && statusFilter === s && <SIcon size={11} />}
        ...
        <div style={{ marginTop: 12, fontSize: 12, color: '#94A3B8', textAlign: 'right', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
          <Layers size={12} color="#CBD5E1" />Showing {filtered.length} of {ALLOCATIONS.length} allocation records
        </div>
      </div>
    </div>
  );
