...
interface Allocation {
  allocationId: number;
  donationRef: string;      // e.g. "DON-001"
...
/* ── Helpers ─────────────────────────────────────────────────── */
const STATUS_CFG: Record<AllocStatus, { bg: string; text: string }> = {
  Allocated: { bg: '#DBEAFE', text: '#1E40AF' },
  Spent:     { bg: '#DCFCE7', text: '#166534' },
      ...
      {houses.map(h => {
        ...
        return (
          <div key={h} style={{
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: '#1E3A5F', marginBottom: 8 }}>{h}</div>
            ...
            <div style={{ background: '#E2E8F0', borderRadius: 4, height: 6, overflow: 'hidden', marginBottom: 8 }}>
              <div style={{ background: '#0D9488', width: `${pct}%`, height: '100%', borderRadius: 4, transition: 'width 0.4s' }} />
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11 }}>
              <span style={{ color: '#166534' }}>💵 Spent ${spent.toLocaleString()}</span>
              <span style={{ color: '#1E40AF' }}>🔒 ${alloc.toLocaleString()} pending</span>
            </div>
          </div>
        );
      })}
...
function ProgramBreakdown({ data }: { data: Allocation[] }) {
  ...
  const rows = programs.map(p => ({
    ...
    spent: data.filter(a => a.programArea === p && a.status === 'Spent').reduce((s, a) => s + a.amountUsd, 0),
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, padding: '20px 24px', border: '1px solid #E2E8F0', boxShadow: '0 2px 12px rgba(30,58,95,0.06)', marginBottom: 24 }}>
      <h6 style={{ fontFamily: 'Poppins,sans-serif', fontWeight: 700, color: '#1E3A5F', marginBottom: 16 }}>Allocation by Program Area</h6>
          ...
          return (
            <div key={r.name}>
              ...
              <div style={{ background: '#F1F5F9', borderRadius: 4, height: 8, overflow: 'hidden' }}>
                <div style={{ background: 'linear-gradient(90deg, #1E3A5F, #0D9488)', width: `${pct}%`, height: '100%', borderRadius: 4, transition: 'width 0.4s' }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="container">
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ color: '#64748B', fontSize: 14, marginBottom: 0 }}>
            Track how donations are allocated and spent across safehouses and program areas.
          </p>
        </div>
        ...
        <div style={{
          ...
          <input type="text" placeholder="Search by supporter or ref…"
          ...
          {['All', ...ALL_STATUSES].map(s => {
            ...
            return (
              <button key={s} onClick={() => setStatusFilter(s)} style={{
                ...
                background: statusFilter === s ? (cfg?.bg ?? '#1E3A5F') : '#E2E8F0',
                color: statusFilter === s ? (cfg?.text ?? '#fff') : '#475569',
                    ...
                    return (
                      <tr key={a.allocationId} style={{ background: i % 2 === 0 ? '#fff' : '#FAFAFA', borderBottom: '1px solid #F1F5F9' }}>
                        ...
                        <td style={{ padding: '12px 16px' }}>
                          <Badge label={a.status} bg={sCfg.bg} text={sCfg.text} />
                        </td>
                        <td style={{ padding: '12px 16px', color: '#64748B', whiteSpace: 'nowrap' }}>
                          {new Date(a.allocationDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </td>
                        <td style={{ padding: '12px 16px', color: '#64748B', whiteSpace: 'nowrap' }}>
                          {a.spentDate ? new Date(a.spentDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                        </td>
                        <td style={{ padding: '12px 16px', color: '#94A3B8', fontSize: 11, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {a.notes ?? '—'}
                        </td>
                      </tr>
                    );
